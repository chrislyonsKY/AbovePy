...
def get_county_bbox(county: str) -> tuple[float, float, float, float]:
    ...
    if normalized not in KY_COUNTIES:
        ...
        raise ValueError(
            "Unknown county '{}'. Use list_counties() to see valid names.".format(county)
        )
...
def validate_bbox(bbox: tuple[float, float, float, float]) -> None:
    ...
    if xmin >= xmax:
        raise ValueError("xmin ({}) must be less than xmax ({})".format(xmin, xmax))
    if ymin >= ymax:
        raise ValueError("ymin ({}) must be less than ymax ({})".format(ymin, ymax))
